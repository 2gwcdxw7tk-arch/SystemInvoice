...
import { ChevronLeft, ChevronRight, LayoutDashboard, ListChecks, PackageSearch, Receipt, Settings, Table, Users, BarChart3, type LucideIcon } from "lucide-react";
    ...
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto pr-1">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link key={key} href={item.href} prefetch aria-current={isActive ? "page" : undefined}
              className={cn("flex min-w-0 items-center gap-3 rounded-2xl border border-transparent px-4 py-3 text-left transition-all duration-200 ease-out",
                "hover:border-primary/40 hover:bg-primary/5",
                isActive ? "border-primary/70 bg-primary/10 text-primary" : "text-muted-foreground",
