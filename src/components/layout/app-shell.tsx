...
function AppShellInner({ children }: { children: ReactNode }) {
  ...
  return hideNavigation ? (
    ...
    <>
      ...
      <div
        className={cn(
          "min-h-screen overflow-x-hidden px-4 pt-4 sm:px-6 lg:grid lg:gap-4 lg:px-10",
          collapsed ? "lg:grid-cols-[5rem,1fr]" : "lg:grid-cols-[16rem,1fr]"
        )}
      >
        <aside className="sticky top-[0rem] hidden self-start lg:block">
          <div className={cn(collapsed ? "w-[5rem]" : "w-[16rem]")}>
        ...
        <main className="flex-1 px-0 pb-10 pt-2 sm:px-2 sm:pt-4 lg:px-4 lg:pt-4">
          <BackToDashboard />
          {children}
        </main>
      </div>
