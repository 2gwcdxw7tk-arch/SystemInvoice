...
## Domain Patterns
- **Facturación**: Flows are separated by `mode` in `src/app/facturacion/page.tsx`. Extiende el switch existente en lugar de crear rutas nuevas. Ninguna factura puede emitirse ni imprimirse cuando el saldo pendiente sea distinto de 0, salvo en el flujo retail con `paymentMode === "CREDITO"`, donde se permite registrar el saldo como crédito y se envía a CxC en estado pendiente.
...
- **Documentos de inventario**: `InventoryTransactionRepository` expone `findTransactionDocumentByCode` y `listTransactionHeaders`; consúmelos desde `InventoryService.getTransactionDocument`/`listTransactionHeaders` para mostrar encabezado + líneas (incluyendo desgloses de `inventory_movements`). El handler `/api/inventario/documentos/[transactionCode]` entrega JSON por defecto y HTML imprimible con `format=html`, mientras que `/api/inventario/documentos` usa los mismos filtros para el listado maestro. La UI de `/inventario/documentos` debe apoyarse en estos endpoints para poblar el foliador, abrir el visor lateral y disparar las impresiones (misma experiencia que los banners post-registro). Evita duplicar lógica de acceso directo a Prisma o a las tablas de `inventory_transactions`.
  - Asegúrate de que `app.inventory_transactions` incluya la columna `updated_at` (migración `20251203103000_inventory_transactions_updated_at`) porque el trigger `trg_inventory_transactions_touch_updated_at` la actualiza en cada `UPDATE`. Si falta, Prisma devuelve `The column "new" does not exist` cuando se recalcula `total_amount`.
- **Consecutivos**: Gestiona las series con `SequenceService`. Las definiciones (`INVOICE`/`INVENTORY`) se crean/actualizan vía `/api/preferencias/consecutivos`, las cajas se enlazan con `/api/preferencias/consecutivos/cajas` y los movimientos de inventario con `/api/preferencias/consecutivos/inventario`. `SequenceService.generateInvoiceNumber` y `generateInventoryCode` son la única fuente de folios; valida que existan asignaciones antes de emitir facturas o transacciones. Cuando varias cajas comparten la misma definición `INVOICE` o distintos movimientos de inventario usan la misma definición `INVENTORY`, el contador se lleva de forma global para evitar reinicios por flujo.
- **Cliente mostrador por caja**: Configura el cliente predeterminado de cada caja desde `/preferencias` → tab **Cajas** cuando `NEXT_PUBLIC_ES_RESTAURANTE=false`. Haz doble clic en la columna de cliente para abrir el buscador (solo lista clientes activos con condición de pago CONTADO). Solo administradores pueden cambiarlo y la API valida que el código exista antes de persistirlo; la UI de facturación es solo de consulta.
- **Environment Variables**: Usa una sola cadena de conexión para toda la app: `DB_CONNECTION_STRING`. Prisma y el runtime leen de la misma variable (se acepta `DATABASE_URL` solo como alias de compatibilidad si faltara). Lee variables a nivel de módulo y memorizalas cuando se reutilicen (e.g., `NEXT_PUBLIC_VAT_RATE`, `DEFAULT_PRICE_LIST_CODE`, `DEFAULT_SALES_WAREHOUSE_CODE`). `NEXT_PUBLIC_CLIENT_LOGO_URL` controla el logotipo mostrado en login y barra superior y `NEXT_PUBLIC_COMPANY_ADDRESS` define la dirección impresa en tickets. `NEXT_PUBLIC_ES_RESTAURANTE` es la bandera maestra para activar/ocultar módulos de restaurantes y debe exponerse vía `env.features`/`publicFeatures` (acepta valores truthy `true|1|yes|on` y falsy `false|0|no|off` para mantener paridad entre servidor y cliente); `LICENSE_MAX_CASH_REGISTERS` define el límite licenciado de cajas y sesiones activas (0 = ilimitado) y se expone en `env.licenses`.
  - `docs/next-public-es-restaurante.md` documenta qué rutas, servicios y cierres dependen de esta bandera; revísalo antes de tocar mesas, meseros o CxC.
- **Reportes de cierre en retail**: Cuando `NEXT_PUBLIC_ES_RESTAURANTE=false`, el HTML de cierre debe incluir la tarjeta "Crédito enviado a CxC" mostrando únicamente el monto total facturado a crédito (sin saldos pendientes). Ajusta la UI/servicios para mantener esta sección sincronizada con `creditTotals`.
- **Permisos CxC**: El módulo de cuentas por cobrar (activo solo cuando `NEXT_PUBLIC_ES_RESTAURANTE=false`) se rige por los permisos `menu.cxc.view`, `customers.manage`, `payment-terms.manage`, `customer.documents.manage`, `customer.documents.apply`, `customer.credit.manage`, `customer.collections.manage` y `customer.disputes.manage`. Los seeds/mocks ya asignan todos al rol `ADMINISTRADOR`. Reusa estos códigos desde `hasSessionPermission` y los guards del middleware.
- **Modelo de datos CxC**: Prisma expone `payment_terms`, `customers`, `customer_documents`, `customer_document_applications`, `customer_credit_lines`, `collection_logs` y `customer_disputes`. Toda integración retail debe generar documentos espejo ligados a `invoices.customer_id/payment_term_id/due_date` y aplicar movimientos mediante los servicios correspondientes (mock + DB).
- **API CxC**: Usa `requireCxCPermissions` (nuevo helper en `src/lib/auth/cxc-access.ts`) para proteger `/api/preferencias/terminos-pago`, `/api/cxc/clientes`, `/api/cxc/documentos` y `/api/cxc/documentos/aplicaciones`. Los handlers deben delegar en `paymentTermService`, `customerService`, `customerDocumentService` y `customerDocumentApplicationService`, respetando `MOCK_DATA` y las validaciones Zod compartidas en los servicios.
- **CxC Documentos manuales**: El panel `Cuentas por Cobrar → Documentos` expone un modal controlado (`CreateDocumentModal`) que carga catálogos bajo demanda, calcula la fecha de vencimiento según la condición de pago y sincroniza automáticamente el saldo con el monto original (manteniendo la posibilidad de overrides). El listado debe respetar filtros combinados por cliente, tipo de documento y rango de fechas; estos filtros se comparten vía query params (`customerId`, `documentType`, `dateFrom`, `dateTo`) para habilitar deep links desde el catálogo de clientes.
...
## Data & Integration
  ...
  - `/api/preferencias/terminos-pago`: CRUD de condiciones de pago retail respaldado por `PaymentTermService`.
  - `/api/cxc/clientes`: Listado + creación/actualización de clientes retail con `CustomerService`.
  - `/api/cxc/documentos` y `/api/cxc/documentos/aplicaciones`: Gestión del ledger CxC (documentos y aplicaciones) vía `CustomerDocumentService` y `CustomerDocumentApplicationService`.
